...
- **Room sounds.** The Ambience panel (sliders icon) has 39 loops from [Moodist](https://github.com/remvze/moodist) in six groups: rain, nature, places, things, animals, noise. Each has its own level; they belong to the room, so they keep going when the record stops. Off by default, remembered per browser, downloaded only when switched on.
